...
def plot(runner, axes, set=colors_set['Kraftime']):
    ...
    # 绘制评价损失变化曲线
    axes[0].plot(runner.dev_batches, runner.dev_loss, color=dev_color, linestyle="--", label="Dev loss")
    # 绘制坐标轴和图例
    ...
    # 绘制评价准确率变化曲线
    axes[1].plot(runner.dev_batches, runner.dev_scores, color=dev_color, linestyle="--", label="Dev accuracy")
    # 绘制坐标轴和图例
